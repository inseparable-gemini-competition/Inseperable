...
export type CommandType = 'read' | 'identify' | 'price' | 'donate' | 'taboo' | 'whattosay' | 'plan' | 'shop' | 'impact' | 'tips' | 'takephoto' | 'goback';
...
export const useCommandHandler = (deps: CommandHandlerDependencies) => {
  ...
  const commandMap = useMemo(() => ({
    ...
    takephoto: () => deps.imageCapture.handleShowCamera({ autoCapture: false }),
    goback: () => navigation.goBack(),
  }), [deps, navigation]);

  const handleCommand = useCallback(async (command: string): Promise<void> => {
    try {
